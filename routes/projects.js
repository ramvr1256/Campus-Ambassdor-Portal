...
//Prof side update a project details
router.post("/updateproject/:id", tokenAuth, isProf, async (req, res) => {
  const professor = await Professor.findOne({ _id: req.user._id });
  //console.log(req.body);
  try {
    const project = await Project.findOne({ _id: req.params.id });
    project.set({ ...req.body, professor: professor });
    await project.save();
    res.redirect("/home");
  } catch (err) {
    res.status(400).send("Please fill the complete information");
...
router.get("/", tokenAuth, isStudent, async (req, res) => {
  try {
    ...
    const projects = await Project.find({ available: true })
      ...
      const allproject= await Project.find({ available: true }).sort({ createdAt: "desc" });
