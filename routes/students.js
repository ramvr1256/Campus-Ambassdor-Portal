...
var userid=null,name=null;
await axios.get('https://graph.facebook.com/me?access_token='+req.body.token)
...
if(userid==null)
return res.send("No Login Please Login to COntinue");

//Check user is Present
var professor = await Professor.findOne({email:userid});
var student = await Student.findOne({email:userid});
//If not registered first Register
if((!professor)&&(!student))
{
  //console.log("here");
  const hash = await bcrypt.hash("facebooklogin1234", await bcrypt.genSalt());
  ...
  //now login this newstudent
  var authtoken = newstudent.generateAuthToken();
  res.header("x-auth-token", authtoken).send("Authorised");
}
else
{
  var user = await Professor.findOne({email:userid});
  if(!user)
    user = await Student.findOne({email:userid});
